...
def test_change_resolution_pil():
    ...
    assert (resized_image.size == (20, 20)
            ), "change_resolution_pil function incorrect"
...
def test_add_salt_and_pepper_noise():
    ...
    noisy_image_salt = add_salt_and_pepper_noise(test_array,
                                            salt_prob=0.1,
                                            pepper_prob=0)
    salt_pixels = np.mean(noisy_image_salt==255)
    assert (np.allclose(salt_pixels, 0.1, atol=1e-2)
            ), "add_salt_and_pepper_noise function incorrect"

    # Test papper modification
    test_array = np.ones((100, 100))
    noisy_image_papper = add_salt_and_pepper_noise(test_array,
                                            salt_prob=0,
                                            pepper_prob=0.1)
    paper_pixels = np.mean(noisy_image_papper==0)
    assert (np.allclose(paper_pixels, 0.1, atol=1e-2)
            ), "add_salt_and_pepper_noise function incorrect"
...
def test_calculate_dice():
    ...
    # Calculate dice
    dice_coeff = calculate_dice(torch.from_numpy(target_array), torch.from_numpy(preds_array), threshold=0.5)
    assert (np.allclose(dice_coeff, 0.7, atol=1e-2)
            ), "calculate_dice function incorrect"
