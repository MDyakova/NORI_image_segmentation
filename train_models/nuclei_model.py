...
from dataset_utilities import make_dataset_directory, train_val_split, save_subset
...
if __name__ == "__main__":
    # Load config
    with open(
        os.path.join(os.path.join("train_directory", "train_config.json")),
        "r",
        encoding="utf-8",
    ) as f:
        config = json.load(f)
    ...
    make_dataset_directory(os.path.join("datasets"), model_name, target_name="masks")
    ...
    # Train model
    model_dir = os.path.join("train_directory", "models")
    unet_train(
        train_loader,
        val_loader,
        model_dir,
