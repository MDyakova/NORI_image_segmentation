...
if __name__ == "__main__":
    ...
    # Load config
    with open(
        os.path.join(os.path.join("inference_directory", "inference_config.json")),
        "r",
        encoding="utf-8",
    ) as f:
        config = json.load(f)
    ...
    nuclei_model.load_state_dict(torch.load(nuclei_model_path, map_location=device))
    nuclei_model.eval()
    ...
    for file_name in image_names:
        ...
        if (".tif" in file_name) & (labels_name not in saved_labels):
            ...
            image_full = Image.fromarray(
                (image_full * 255).transpose((1, 2, 0)).astype(np.uint8)
            )
            ...
            for step_i in range(0, height - crop_size // 2, crop_size // 2):
                for step_j in range(0, width - crop_size // 2, crop_size // 2):
                    ...
                    image_crop = (
                        (image_crop * 255).transpose((1, 2, 0)).astype(np.uint8)
                    )
                    ...
                    with torch.no_grad():
                        ...
                        nuclei_results = (
                            nuclei_results > nuclei_prob
                        ).float()  # Binarize prediction
                        nuclei_results = nuclei_results.squeeze().cpu().numpy()
                    nuclei_results = np.array(
                        Image.fromarray(nuclei_results.astype(np.uint8) * 255).resize(
                            (width_crop, height_crop)
                        )
                    )

                    # Add tubule polygons to whole masks
                    (mask_for_tubules, mask_for_prob, mask_n) = tubule_contours(
                        tubule_results,
                        mask_for_tubules,
                        mask_for_prob,
                        tubule_prob,
            ...
            polygons_save = pd.DataFrame(
                polygons_save, columns=["file_name", "type", "x", "y"]
            )
            polygons_save.to_csv(
                os.path.join(labels_save, file_name_save + ".csv"), sep=";", index=False
            )

            # Found lumen for each tubule contour
            mask_for_lumen = lumen_predict(
                image_full,
                all_masks,
                lumen_coef,
                distance_threshold,
            ...
            # Save control image
            file_path_save = os.path.join(images_save, file_name_save + ".png")
