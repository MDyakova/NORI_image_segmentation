...
import torch
from utilities import (image_filter,
...
def test_image_filter():
    ...
    assert (np.allclose(np.mean(test_array[0])/1000, np.mean(filtered_image[0]), atol=1e-2)
...
def test_image_to_unet():
    ...
    assert (list(transformed_image.shape )== [1, 3, 640, 640]
            ), "image_to_unet function incorrect"

    assert (transformed_image[0][0].mean() < 1
        ), "image_to_unet function incorrect"
...
def test_find_similar_contours_fast():
    ...
    assert ((len(unique_id[0]) == 3) & (unique_id[1][1]==11500)
